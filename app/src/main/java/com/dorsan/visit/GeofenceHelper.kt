...
class GeofenceHelper(private val context: Context) {
    ...
    // onResult reçoit soit null (succès), soit un message d'erreur lisible.
    // Comme on n'a pas de logcat sous la main dans ce workflow, on remonte l'erreur
    // directement dans l'UI de MainActivity.
    @SuppressLint("MissingPermission")
    fun registerGeofences(pois: List<Poi>, onResult: (String?) -> Unit) {
        ...
        geofencingClient.addGeofences(request, geofencePendingIntent)
            ...
            .addOnFailureListener { e -> onResult(e.message ?: e.toString()) }
    }
}
